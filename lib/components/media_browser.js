...
zuix.controller(function(cp){
    ...
    cp.create = function() {
        ...
        zuix.context(cp.field('carousel'), function (ctx) {
            ctx.on('page:change', function (e, page) {
                currentPage = page.in;
                ctx.get(page.in).children().eq(0).addClass('page-active');
                if (page.out !== -1) {
                    console.log("!!!!", page.in, page.out);
                    ctx.get(page.out).children().eq(0).removeClass('page-active');
                }
            });
        });
