...
zuix.controller(function (cp) {
    ...
    function flyTo(targetPoint, transition) {
        ...
        if (layoutType === LAYOUT_HORIZONTAL) {
            ...
            if (firstData.transform.translate.x + targetPoint.x > 0) {
                x = -firstData.transform.translate.x;
            } else {
                if (lastData.transform.translate.x + lastPage.get().offsetWidth + targetPoint.x < viewSize.width) {
                    x = -spec.marginLeft*2 + viewSize.width - (lastData.transform.translate.x + lastPage.get().offsetWidth);
                }
            }
            ...
            if (firstData.transform.translate.y + targetPoint.y > 0) {
                y = -firstData.transform.translate.y;
            } else {
                if (lastData.transform.translate.y + lastPage.get().offsetHeight + targetPoint.y < viewSize.height) {
                    y = -spec.marginTop*2 + viewSize.height - (lastData.transform.translate.y + lastPage.get().offsetHeight);
                }
            }
    ...
    function dragShift(x, y, transition) {
        if (transition != null)
            transition = transition.duration+'s '+transition.easing;
        pageList.each(function (i,el) {
            const data = getData(this);
    ...
    function handleSwipe(e, tp) {
        const fastSwipe = (!enablePaging && Math.abs(tp.velocity) > 0.45) || (Math.abs(tp.velocity) > 1.25);
        const decelerateEasing = {
            duration: (.5+(1/Math.abs(tp.velocity))),
            easing: 'cubic-bezier(0.2,0.5,0.3,1.15)'
        };
        const ap = {
            x: (tp.velocity * 1000),
            y: (tp.velocity * 1000)
