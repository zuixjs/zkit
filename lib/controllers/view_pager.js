...
zuix.controller(function (cp) {
        ...
        zuix.load('//genielabs.github.io/zkit/lib/controllers/gesture_helper', {
            ...
            on: {
                ...
                'gesture:swipe': function (e, tp) {
                    if (Math.abs(tp.velocity) > 1.25) {
                        var decelerateEasing = '0.75s cubic-bezier(.2, .5, .15, 1.15)';
                        if (layoutType === LAYOUT_HORIZONTAL) {
                            focusPageAt({
                                x: tp.x - (tp.velocity * 1200),
                                y: tp.y
                            }, decelerateEasing);
                        } else {
                            focusPageAt({
                                x: tp.x,
                                y: tp.y - (tp.velocity * 1200)
                            }, decelerateEasing);
                        }
    ...
    function focusPageAt(tp, transition) {
        ...
        setPage(currentPage, transition != null ? transition : DEFAULT_PAGE_TRANSITION);
    }
