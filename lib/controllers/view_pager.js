...
zuix.controller(function (cp) {
    ...
    const LAYOUT_HORIZONTAL = 1, LAYOUT_VERTICAL = 2;
    const SLIDE_DIRECTION_FORWARD = 1, SLIDE_DIRECTION_BACKWARD = -1;
    let currentPage = -1, oldPage = 0;
    let slideTimeout = null, slideInterval = 3000;
    ...
    function handleSwipe(e, tp) {
        ...
        const decelerateEasing = (1/Math.abs(tp.velocity))+'s cubic-bezier(.1, .2, .15, 1.15)';
        const ap = {
            x: (tp.velocity * 1000),
            y: (tp.velocity * 1000)
        };
