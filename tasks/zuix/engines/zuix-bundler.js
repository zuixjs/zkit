  ...
  if (zuixConfig.build.bundle && zuixConfig.build.bundle.css) {
    ...
    if (styleList != null) {
      styleList.forEach(function(el) {
        ...
        const cssText = fetchResource(resolveResourcePath(page.file, resourcePath), false);
        if (cssText != null) {
          el.outerHTML = '<style>\n/*{% raw %}*/\n'+cssText+'\n/*{% endraw %}*/\n</style>';
  ...
  if (zuixConfig.build.bundle.zuix !== false) {
    ...
    if (nodeList != null) {
      nodeList.forEach(function(el) {
        ...
        let hasJsFile = false;
        let resourcePath = el.getAttribute('data-ui-include') || el.getAttribute('z-include');
        if (resourcePath == null || resourcePath === '') {
          hasJsFile = true;
          resourcePath = el.getAttribute('data-ui-load') || el.getAttribute('z-load');
        }
        // do not process inline views
        if (dom.window.document.querySelectorAll('[data-ui-view="' + resourcePath + '"],[z-view="' + resourcePath + '"]').length > 0) {
          return;
        }
        ...
        if (content != null) {
          ...
          if (el.getAttribute('data-ui-mode') === 'unwrap' || el.getAttribute('z-mode') === 'unwrap') {
...
module.exports = function(options, template, page, cb) {
  ...
  const html = staticSite.swig(page, localVars)._result.contents;
  const isStaticSite = (html != page.content);
  if (isStaticSite) {
    page.content = html;
  }
