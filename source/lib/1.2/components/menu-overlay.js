...
function MenuOverlay(cp) {
  ...
  cp.create = function() {
    menuPosition = cp.options().position || menuPosition;
    ...
    const $view = cp.view();
    const buttonColor = cp.options().buttonColor; // eg. header
    if (buttonColor != null) {
      $view.find('.circle-button').css({background: buttonColor});
    }
    const iconColor = cp.options().iconColor; // eg. header
    if (iconColor != null) {
    ...
    const scrollerName = cp.options().scroller;
    ...
    if (scroller != null) {
      let beforeElement = cp.options().before; // eg. footer
      if (beforeElement) {
        beforeElement = zuix.field(beforeElement).get();
      }
      let afterElement = cp.options().after; // eg. header
      if (afterElement) {
